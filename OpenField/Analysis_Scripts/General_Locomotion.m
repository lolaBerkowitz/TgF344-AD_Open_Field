...
for i = 1:length(params.subID)
    ...
    move = [velocity(1); velocity] > 3;
    edges = (0:5:30)*60;
    for bin = 1:6
        idx = ts > edges(bin) & ts <= edges(bin+1);
        path_length_time(i,bin) = sum(distance_vector(move & idx));
    end
    
    % Distance for first 10 min (for comparison with small OF) 
    move = [velocity(1); velocity] > 3;
    edges = [0 600];
    idx = ts > edges(1) & ts <= edges(2);
    path_length_first_ten(i,1) = sum(distance_vector(move & idx));
...
vars = {'subID','group','day','path_length',...
    ...
    'path_length_tim5',...
    'path_length_tim6',...
    'path_length_first_10',...
    'search_area','thigmotaxis'};

df = table(subID,group,day,path_length,median_velocity,median_acceleration,...
    median_angular_velocity,path_length_time(:,1),...
    path_length_time(:,2),...
    path_length_time(:,3),...
    path_length_time(:,4),...
    path_length_time(:,5),...
    path_length_time(:,6),...
    path_length_first_ten,...
    search_area,out,'VariableNames',vars);
