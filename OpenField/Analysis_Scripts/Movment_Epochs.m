...
for i = 1:length(params.subID)
    ...
    for ii = 1:length(events)
        ...
        run{ii} = [x(idx),y(idx)];
        head{ii} = [head_x(idx),head_y(idx)];
        neck{ii} = [neck_x(idx),neck_y(idx)];
    end
    ...
    for iii = 1:size(run,2)
        ...
        if isnan(cue_cords)
            ...
            run_cue_bearing(iii) = cue_heading(head{iii},neck{iii},params.cueCM{i - 1});
        else
            run_cue_proximity(iii)  = proximity_to_cue(run{iii}(:,1),run{iii}(:,2),cue_cords);
            run_cue_bearing(iii) = cue_heading(head{iii},neck{iii},cue_cords);
    ...
    params.peak_vel{i} = peak_vel;
    params.run_duration{i} = duration;
    params.cue_proximity{i} = run_cue_proximity;
    params.cue_bearing{i} = run_cue_bearing;
    ...
    clear run run_length run_circuity run_cue_bearing run_cue_proximity
...
function mean_bearing = cue_heading(Head_coords,Neck_Coords,cue_coords)
% Head direction 
hd = wrapTo360(rad2deg(atan2(Neck_Coords(:,2) - Head_coords(:,2),...
    Neck_Coords(:,1) - Head_coords(:,1))));

% egocentric head bearing
polyin = polyshape(cue_coords(:,1),cue_coords(:,2));
[x,y] = centroid(polyin);

head_bearing = wrapTo360(rad2deg(atan2(y-Head_coords(:,2),...
    x-Head_coords(:,1))));

head_bearing = wrapTo360(head_bearing-hd);

mean_bearing = rad2deg(circ_mean(deg2rad(head_bearing)));
end
